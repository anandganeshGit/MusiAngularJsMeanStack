...
//Track Model
let TrackInfo = require('../model/track-info');

// Register Form
// router.get('/register', function(req, res){
//     //res.sendFile('../src/app/components/register-user/register-user.component.html');
//     res.render('register-user.component.html', { root: '../src/app/components/register-user/'});
//     //res.send(`${process.env.PWD}/register-user.component.html`);
//   });
...
/*MongoClient.connect(url, function(err, db) {
  ...
  console.log(JSON.stringify(req.body[0].mail).replace(/\"/g, ""));
  var query = { email: 'anandganesh.bala@gmail.com' /!*JSON.stringify(req.body[0].mail).replace(/\"/g, "").toString()*!/ };
  db.db("music1").collection("users").find(query).toArray(function(err, results) {
    if (err) throw err;
    console.log(results);
